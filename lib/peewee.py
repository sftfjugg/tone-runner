...
class OnConflict(Node):
    ...
    def where(self, *expressions):
        ...
        expressions = Query.set_is_deleted_express(expressions)
...
class Query(BaseQuery):
    ...
    @staticmethod
    def set_is_deleted_express(expressions):
        # 增加了默认is_deleted为False的过滤
        is_deleted = "is_deleted"
        for express in expressions:
            if isinstance(express.lhs, Expression):
                return expressions
            if express.lhs.name == is_deleted:
                return expressions
        expressions = list(expressions)
        is_deleted_exp = deepcopy(expressions[0])
        setattr(is_deleted_exp, "op", "=")
        setattr(is_deleted_exp, "rhs", False)
        lhs = getattr(is_deleted_exp, 'lhs')
        setattr(lhs, "name", is_deleted)
        setattr(lhs, "safe_name", is_deleted)
        setattr(lhs, "column_name", is_deleted)
        setattr(is_deleted_exp, 'lhs', lhs)
        expressions.append(is_deleted_exp)
        expressions = tuple(expressions)
        return expressions

    @Node.copy
    def where(self, *expressions, filter_is_deleted=True):
        if self._where is not None:
            expressions = (self._where,) + expressions
        if filter_is_deleted:
            expressions = self.set_is_deleted_express(expressions)
        self._where = reduce(operator.and_, expressions)
...
class Model(with_metaclass(ModelBase, Node)):
    ...
    def save(self, force_insert=False, only=None):
        ...
        if pk_value is not None and not force_insert:
            ...
            rows = self.update(**field_dict).where(self._pk_expr(), filter_is_deleted=False).execute()
...
class ModelSelect(BaseModelSelect, Select):
    ...
    def _check_is_deleted(self, dq_node):
        if isinstance(dq_node.lhs, Expression):
            if self._check_is_deleted(dq_node.lhs):
                return self._check_is_deleted(dq_node.lhs)
            if self._check_is_deleted(dq_node.rhs):
                return self._check_is_deleted(dq_node.rhs)
        else:
            return dq_node.lhs.name == "is_deleted"

    def _get_real_node(self, dq_node):
        if isinstance(dq_node.lhs, Expression):
            if self._get_real_node(dq_node.lhs):
                return self._get_real_node(dq_node.lhs)
        else:
            return dq_node
    ...
    def filter(self, *args, **kwargs):
        ...
        dq_node = ColumnBase()
        if args:
            # 增加is_deleted为false的过滤
            for filter_node in args:
                if self._check_is_deleted(filter_node):
                    break
            else:
                args = list(args)
                first_node = deepcopy(args[0])
                is_deleted_node = self._get_real_node(first_node)
                is_deleted_node.lhs.name = "is_deleted"
                is_deleted_node.lhs.safe_name = "is_deleted"
                is_deleted_node.lhs.column_name = "is_deleted"
                is_deleted_node.lhs.default = False
                is_deleted_node.op = "="
                is_deleted_node.rhs = False
                args.append(is_deleted_node)
                args = tuple(args)
